...
public partial class Mainmenu : Control
{
	...
	private void OnLocalButtonPressed()
	{
		...
		var body = Json.Stringify(new Dictionary
		{
			...
			{ "gold", _characterManager.LoadGold() }
		});
		...
		var err = _unlockRequest.Request(
			$"{ServerConfig.BaseUrl}/api/v1/protected/progress/sync",
			headers,
			HttpClient.Method.Post,
			body
		);
	...
	private bool GoldIsEqual(int gold)
	{
		var localGold = _characterManager.LoadGold();
		if (localGold == gold) return true;
		GD.Print($"Gold mismatch: Local: {localGold}, Online: {gold}");
		return false;
	}
