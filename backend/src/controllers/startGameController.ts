...
import {getPodManifest, getServiceManifest, k8sApi, namespace, patchIngress} from "services/k8sService";
...
export async function startGameController(req: Request, res: Response) {
    ...
    try {
        ...
        // Create Service
        const {serviceManifest, nodePort} = getServiceManifest(code, `svc-${code}`)
        ...
        res.status(201).json({message: `Pod ${podName} created with code ${code} on port ${nodePort}`, code});
